...
const DEEP = [
  {
    ...
    points: ["Repeat order lists kept on file", "Reel and drop cable in quantity", "Radio and CPE stock for expansion"],
  },
  {
    ...
    <>
      <Section className="pt-10 sm:pt-16">
        <div className="rise">
          ...
          <Lead className="mt-5">
            An ISP buying a reel every week does not want the same process as a county office running a tender. These are
            the patterns the desk is set up for.
          </Lead>
          ...
          {DEEP.map((item, i) => (
            <Reveal key={item.title} delay={i * 70}>
              <Panel className="h-full">
                <p className="text-xs font-semibold uppercase tracking-[0.16em] text-ember">{item.need}</p>
                <h2 className="mt-2 text-xl font-semibold">{item.title}</h2>
                <p className="mt-3 text-sm leading-relaxed text-muted-foreground">{item.body}</p>
                <ul className="mt-5 flex flex-wrap gap-2">
                  {item.points.map((p) => (
                    <li key={p} className="rounded-full bg-primary/10 px-3 py-1 text-xs font-medium text-primary">
                      {p}
                    </li>
                  ))}
                </ul>
        ...
        <Reveal>
          <div className="gloss rounded-[2rem] p-8 sm:p-12">
            <div className="relative z-10">
              ...
              <Lead className="mt-4">
                Hotels, hospitals, manufacturers, churches and estates all buy from the same shelves. Describe the site
                and the technical desk will work out which of the four supply lines it touches.
              </Lead>
